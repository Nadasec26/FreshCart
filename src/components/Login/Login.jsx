...
let validate = Yup.object().shape({
  ...
  <div className="relative z-0 w-full mb-5 group">
   ...
   {formik.errors.password && formik.touched.password ? <div className="mr-96 text-red-800" role="alert">{formik.errors.password}</div> : null } 
    ...
    {isLoadind ? <i className='fas fa-spinner fa-spin'></i> : "Login"}
  </button>
  <span><Link to={"/ForgetPassward"} className='text-emerald-600 underline hover:text-emerald-700 hover:underline'>Forget Password?</Link> </span>
    </div>
  </form>
  </div>
  </>
