...
export default function Cart() {
  ...
  let {getLoggedCart, updateCartProduct,deleteCartItems,  numberItems, setnumberItems} = useContext(CartContext);
  ...
  {DetailCart?.products.length > 0 ? <>
    <h2 className='text-2xl text-emerald-600 font-bold'>Total Price : {DetailCart?.totalCartPrice}</h2>
<div className="relative overflow-x-auto shadow-md sm:rounded-lg">
  <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
    ...
    <tbody>
      {DetailCart?.products.map((product) => 
  ...
  </> : <h1 className='capitalize text-red-800 font-bold text-3xl my-8'>no product added</h1>}
